...
function BecomeAMentor(props) {
    ...
    const becomeAMentor = () => {
        ...
        if (user.mid) {
            API.editTimetable(user.id, selectedRows).then((response) => {}).catch((err)=>console.log(err));

        } else {
            API.addMentor(user.id, { year: newYear, courses: newTeachings.map(t => courses[t.id] + ":" + t.grade).join(", ") }).then((response) => {
                API.editTimetable(user.id, selectedRows).then((response) => {
                    API.getMentorsData().then((data) => {
                        ...
                        if (mentor) {
                            setUser({ ...user, ...mentor });
                            setIsTeacher(true);
                            Swal.fire('Congratulations! you are now a mentor', '', 'success');
                            navigate('/Home');
                        }
        ...
        <Row style={{ padding: "20px" }} className='form-fill'>
            ...
            <Col >
                ...
                <Row>
                    <Col md="12">
                        {user.mid ?
                            <Button type="submit" variant="success" onClick={() => Swal.fire('NOT IMPLEMENTED', '', 'error')}>Save your mentor info</Button> :
                            <Button type="submit" variant="success" onClick={becomeAMentor}>Become a mentor</Button>
                        }
                    </Col>
                </Row>
            </Col>
